...
std::vector<glm::vec3> buildCircle(float x, float y, float radius) {
  ...
  for (int i = 0; i <= vertexCount; i++) { 
    ...
    float vx = x + radius * cos(glm::radians(currentAngle));
    float vy = y + radius * sin(glm::radians(currentAngle));
    vertices.push_back(glm::vec3(vx, vy, 0.0f));
  }
...
int main()
{
    ...
    std::vector<glm::vec3> vertices = buildCircle(0.0f, 0.0f, 0.5f);
